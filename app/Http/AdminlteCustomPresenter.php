...
class AdminlteCustomPresenter extends Presenter
{
    ...
    public function getMenuWithDropDownWrapper($item)
    {
        ...
        $this->formatIcon($item->icon) . ' <span class="tw-truncate">' . $item->title . '</span>' .
        '<svg aria-hidden="true" class="svg tw-text-gray-500 tw-size-4 tw-shrink-0" style="margin-inline-start:auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">' . $this->getArray($item) .
            '</svg>' .
            '</a>';
    ...
    public function getChildMenuItems($item)
    {
        ...
        if (count($item->getChilds()) > 0) {
            ...
            // Use logical properties (padding-inline-start / inset-inline-start) so the
            // child indent and the vertical guide line flip automatically under RTL (dir=rtl).
            $children .= '<div class="chiled tw-relative tw-mt-1 tw-mb-2" style="padding-inline-start:2.75rem;display:' . $displayStyle . '">
            <div class="tw-absolute tw-inset-y-0 tw-w-px tw-h-full tw-bg-gray-100" style="inset-inline-start:1.25rem"></div>
