    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                || auth()->user()->can('expense_report.view') || auth()->user()->can('walkin.view') || auth()->user()->can('walkin.view_all')) {
                $menu->dropdown(
                    __('report.reports'),
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('profit_loss_report.view')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getZReport']),
                                __('report.z_report'),
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'purchaseSaleProductReport']),
                                __('lang_v1.purchase_sale_product_report'),
                                ['icon' => '', 'active' => request()->segment(2) == 'purchase-sale-product']
                            );
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'paymentByAgeReport']),
                                __('lang_v1.payment_by_age_report'),
                                ['icon' => '', 'active' => request()->segment(2) == 'payment-by-age-report']
                            );
                        }
            ...
            if (config('recommerce.enabled', false)
                && Route::has('recommerce.customer_repairs.index')
                && (auth()->user()->can('recommerce.repair.view') || auth()->user()->can('recommerce.repair.intake'))) {
                $menu->dropdown(
                    'Customer Repairs',
                    function ($sub) {
                        if (auth()->user()->can('recommerce.repair.view') || auth()->user()->can('recommerce.repair.intake')) {
                            $sub->url(
                                route('recommerce.customer_repairs.index'),
                                'Customer repair queue',
                                ['icon' => '', 'active' => request()->segment(1) == 'recommerce' && request()->segment(2) == 'customer-repairs']
                            );
                        }
                        if (auth()->user()->can('recommerce.repair.intake')) {
                            $sub->url(
                                route('recommerce.repair.new'),
                                'New customer repair',
                                ['icon' => '', 'active' => request()->segment(1) == 'recommerce' && request()->segment(2) == 'repair' && request()->segment(3) == 'new']
                            );
                        }
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M14.7 6.3a1 1 0 0 0 -1.4 0l-7 7a1 1 0 0 0 0 1.4l2.6 2.6a1 1 0 0 0 1.4 0l7 -7a1 1 0 0 0 0 -1.4z"></path><path d="M16 4l4 4"></path><path d="M12 8l4 4"></path><path d="M3 21l3 -3"></path></svg>']
                )->order(27);
            }
