...
class ReportController extends Controller
{
    ...
    /**
     * Shows the Z report (Total Sales in Hand) of a business.
     *
     * Reuses the same TransactionUtil aggregates as the Profit/Loss & Purchase-Sell
     * reports. All figures are tax-inclusive. "Total Sales" is shown gross (before
     * order discount) so the "Less: Discounts" line is a genuine deduction:
     *   Total Sales in Hand = Total Sales - Sell Returns - Purchases
     *                         + Purchase Returns - Discounts - Expenses
     *
     * @return \Illuminate\Http\Response
     */
    public function getZReport(Request $request)
    {
        if (! auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            //Total sold amount (tax inclusive)
            ...
            //Total purchase amount (tax inclusive)
            ...
            //Sell/purchase returns, sell discount & expenses
            $transaction_totals = $this->transactionUtil->getTransactionTotals(
                $business_id,
                ['sell', 'purchase', 'sell_return', 'purchase_return', 'expense'],
                $start_date,
                $end_date,
                $location_id,
                null,
            ...
            $total_discount = $transaction_totals['total_sell_discount'];
            $total_purchase_discount = $transaction_totals['total_purchase_discount'];

            //Inline/line-level discounts.
            $sell_line_discount = $this->transactionUtil->getTotalSellLineDiscount($business_id, $start_date, $end_date, $location_id, null, $permitted_locations);
            $purchase_line_discount = $this->transactionUtil->getTotalPurchaseLineDiscount($business_id, $start_date, $end_date, $location_id, null, $permitted_locations);

            $data = [
                //Gross item sales without tax. "Incl. Discount" => the discount is INCLUDED here
                //(not subtracted); it is applied later in the total. = total_before_tax + line discount.
                'total_item_sales' => $sell_details['total_sell_exc_tax'] + $sell_line_discount,
                //Full discount contained in the figure above (line + invoice)
                'sell_discount' => $total_discount + $sell_line_discount,
                'total_sell_return' => $transaction_totals['total_sell_return_inc_tax'],
                //Gross purchase without tax (discount included, applied later in the total)
                'total_purchase' => $purchase_details['total_purchase_exc_tax'] + $purchase_line_discount,
                //Full discount contained in the purchase figure above (line + invoice)
                'purchase_discount' => $total_purchase_discount + $purchase_line_discount,
                'total_purchase_return' => $transaction_totals['total_purchase_return_inc_tax'],
                //Delivery/shipping charges collected on sells
                'total_delivery_charge' => $sell_details['total_shipping_charges'],
                'total_expense' => $transaction_totals['total_expense'],
            ];

            //Discounts are applied here (figures above are gross/discount-inclusive).
            $data['total_sales_in_hand'] = $data['total_item_sales']
                - $data['sell_discount']
            ...
            return view('report.partials.z_report_data', compact('data'))->render();
        }

        $business_locations = BusinessLocation::forDropdown($business_id, true);

        return view('report.z_report', compact('business_locations'));
    }
        ...
        if ($request->ajax()) {
            ...
            // remove above line from select and below join becouse customer search not work
                ...
                ->select(
                    ...
                    't.custom_field_3',
                    't.custom_field_4',
                    DB::raw('DATEDIFF(DATE(transaction_payments.paid_on), DATE(t.transaction_date)) as age')
                )
                ->groupBy('transaction_payments.id');
            ...
            if (! empty($request->get('custom_field_1'))) {
                $query->where('t.custom_field_1', 'like', '%'.$request->get('custom_field_1').'%');
            }
            if (! empty($request->get('custom_field_2'))) {
                $query->where('t.custom_field_2', 'like', '%'.$request->get('custom_field_2').'%');
            }
            if (! empty($request->get('custom_field_3'))) {
                $query->where('t.custom_field_3', 'like', '%'.$request->get('custom_field_3').'%');
            }
            if (! empty($request->get('custom_field_4'))) {
                $query->where('t.custom_field_4', 'like', '%'.$request->get('custom_field_4').'%');
            }

            if (! empty($request->get('age_filter'))) {
                $age_filter = $request->get('age_filter');
                $age_ranges = [
                    '0-15'  => [0, 15],
                    '16-30' => [16, 30],
                    '31-45' => [31, 45],
                ...
                if (isset($age_ranges[$age_filter])) {
                    [$min, $max] = $age_ranges[$age_filter];
                    $query->whereRaw('DATEDIFF(DATE(transaction_payments.paid_on), DATE(t.transaction_date)) BETWEEN ? AND ?', [$min, $max]);
                } elseif ($age_filter === '90+') {
                    $query->whereRaw('DATEDIFF(DATE(transaction_payments.paid_on), DATE(t.transaction_date)) > 90');
                }
            }
    ...
    /**
     * Age buckets (in days) used by the "Payment by Age" report. The age of a
     * payment is the number of days from the payment date up to today
     * (DATEDIFF(today, paid_on)) — i.e. how long ago the payment was received.
     * The first bucket is open ended downward (it also catches any future-dated
     * payment, i.e. negative age) and the last bucket is open ended upward, so
     * every payment falls into exactly one bucket.
     *
     * @return array
     */
    private function paymentAgeBuckets()
    {
        return [
            '1-15'  => [1, 15],
            '15-30' => [16, 30],
    ...
    /**
     * Age ranges (in days) offered in the "Payment by Age" report's age filter
     * dropdown. These are intentionally separate from the displayed column
     * buckets: the filter narrows the rows to payments whose age falls in the
     * chosen range, while the columns still group by paymentAgeBuckets().
     * The last range ('90+') is open ended upward.
     *
     * @return array
     */
    private function paymentAgeFilterRanges()
    {
        return [
            '0-15'  => [0, 15],
            '16-30' => [16, 30],
    ...
    /**
     * Shows the "Payment by Age" report as a server-side datatable. Each row is
     * a customer ("Payment for") + payment method + user combination; the age
     * bucket columns hold the total amount paid whose age falls in that bucket,
     * with a Total column and a footer that totals every column across the full
     * filtered set. By default all payment methods are shown; filters (customer,
     * payment method, business location, user, age and date range) narrow the result.
     *
     * @return \Illuminate\Http\Response
     */
    public function paymentByAgeReport(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $business_id = $request->session()->get('user.business_id');
        $payment_types = $this->transactionUtil->payment_types(null, true, $business_id);
        $buckets = $this->paymentAgeBuckets();
        $age_filter_ranges = $this->paymentAgeFilterRanges();

        // Stable column alias per bucket (e.g. '16-30' => 'bucket_16_30', '90+' => 'bucket_90plus').
        $bucket_cols = [];
        foreach (array_keys($buckets) as $key) {
            $bucket_cols[$key] = 'bucket_'.str_replace(['-', '+'], ['_', 'plus'], $key);
        }

        if ($request->ajax()) {
            // Age is counted from today back to the payment date, i.e. how many
            // days ago the payment was received (not the sale-to-payment gap).
            $today = \Carbon::now()->format('Y-m-d');
            $age_expr = "DATEDIFF('$today', DATE(transaction_payments.paid_on))";
            // is_return payments are deductions (change/return), so subtract them.
            $amount_expr = 'IF(transaction_payments.is_return = 1, -1 * transaction_payments.amount, transaction_payments.amount)';

            // Base query (joins + filters) shared by the row query and the footer-totals query.
            $base = TransactionPayment::join('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['sell', 'opening_balance']);
            })
                ...
                ->leftjoin('users as u', 'u.id', '=', 'transaction_payments.created_by')
                ->where('transaction_payments.business_id', $business_id)
                ->whereNotNull('transaction_payments.paid_on');

            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            if (! empty($start_date) && ! empty($end_date)) {
                $base->whereBetween(DB::raw('date(transaction_payments.paid_on)'), [$start_date, $end_date]);
            }

            $permitted_locations = auth()->user()->permitted_locations();
            if ($permitted_locations != 'all') {
                $base->whereIn('t.location_id', $permitted_locations);
            }

            if (! empty($request->get('location_id'))) {
                $base->where('t.location_id', $request->get('location_id'));
            }

            if (! empty($request->get('payment_types'))) {
                $base->where('transaction_payments.method', $request->get('payment_types'));
            }

            if (! empty($request->get('user_id'))) {
                $base->where('transaction_payments.created_by', $request->get('user_id'));
            }

            if (! empty($request->get('customer_id'))) {
                $base->where('t.contact_id', $request->get('customer_id'));
            }

            // Age filter: narrow to payments whose age (today - paid_on) is in the chosen range.
            if (! empty($request->get('age_filter')) && isset($age_filter_ranges[$request->get('age_filter')])) {
                [$af_min, $af_max] = $age_filter_ranges[$request->get('age_filter')];
                if (is_null($af_max)) {
                    $base->whereRaw("$age_expr >= ?", [$af_min]);
                } else {
                    $base->whereRaw("$age_expr BETWEEN ? AND ?", [$af_min, $af_max]);
                }
            }

            // SUM(...) expression per bucket, reused by both the row and totals queries.
            // The first and last buckets are open ended so every payment lands in
            // exactly one bucket (and the bucket columns always add up to Total):
            //   - first bucket captures age <= its upper bound, so advance payments
            //     made before the sale date (negative age) count as "current".
            //   - last bucket captures age >= its lower bound.
            $first_key = array_key_first($buckets);
            $bucket_selects = [];
            foreach ($buckets as $key => $range) {
                $col = $bucket_cols[$key];
                if (is_null($range[1])) {
                    $cond = "$age_expr >= {$range[0]}";
                } elseif ($key === $first_key) {
                    $cond = "$age_expr <= {$range[1]}";
                } else {
                    $cond = "$age_expr BETWEEN {$range[0]} AND {$range[1]}";
                }
                $bucket_selects[$col] = "SUM(IF($cond, $amount_expr, 0))";
            }

            // Row query: one row per customer + payment method + user.
            $select = [
                't.contact_id',
                DB::raw("CONCAT(COALESCE(CONCAT(c.supplier_business_name, ' - '), ''), COALESCE(c.name, '')) as customer"),
                'transaction_payments.method',
                DB::raw("CONCAT(COALESCE(u.surname, ''), ' ', COALESCE(u.first_name, ''), ' ', COALESCE(u.last_name, '')) as user_name"),
                DB::raw("SUM($amount_expr) as total"),
            ];
            foreach ($bucket_selects as $col => $expr) {
                $select[] = DB::raw("$expr as $col");
            }

            $query = (clone $base)
                ->select($select)
                ->groupBy('transaction_payments.method', 't.contact_id', 'transaction_payments.created_by');

            // Footer totals over the whole filtered set (not just the current page).
            $totals_select = [DB::raw("SUM($amount_expr) as total")];
            foreach ($bucket_selects as $col => $expr) {
                $totals_select[] = DB::raw("$expr as $col");
            }
            $totals = (clone $base)->select($totals_select)->first();

            $footer = [
                'buckets' => [],
                'total' => $this->transactionUtil->num_f((float) ($totals->total ?? 0)),
            ];
            foreach ($bucket_cols as $key => $col) {
                $footer['buckets'][$key] = $this->transactionUtil->num_f((float) ($totals->$col ?? 0));
            }
            ...
            $datatable = Datatables::of($query)
                ...
                ->editColumn('total', function ($row) {
                    return $this->transactionUtil->num_f((float) $row->total);
                });

            // Blank cells (instead of 0.00) read cleaner, matching the printed report.
            foreach ($bucket_cols as $key => $col) {
                $datatable->editColumn($col, function ($row) use ($col) {
                    $val = (float) $row->$col;

                    return $val != 0 ? $this->transactionUtil->num_f($val) : '';
                });
            }
        ...
        // Build the age filter dropdown options (key '0-15' => label '0-15 days').
        $age_filter_options = [];
        foreach (array_keys($age_filter_ranges) as $key) {
            $age_filter_options[$key] = $key.' '.__('lang_v1.days');
        }

        return view('report.payment_by_age_report')
            ->with(compact('business_locations', 'payment_types', 'buckets', 'bucket_cols', 'users', 'customers', 'age_filter_options'));
    }
    ...
    /**
     * Shows the "Purchase & Sale Product" report landing page.
     *
     * The page exposes three grouping buttons (Category, Brand, Supplier) and a
     * period selector. The matrix itself is loaded over ajax from
     * purchaseSaleProductReportData().
     *
     * @return \Illuminate\Http\Response
     */
    public function purchaseSaleProductReport(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('report.purchase_sale_product_report')
            ->with(compact('business_locations', 'categories', 'brands', 'suppliers'));
    }

    /**
     * Builds the purchase vs sale matrix grouped by category/brand/supplier and
     * returns it as a rendered html partial.
     *
     * Columns are the last N months (3/6/12) each split into Purchase and Sales
     * quantities, preceded by the product's current stock balance.
     *
     * @return \Illuminate\Http\Response
     */
    public function purchaseSaleProductReportData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $group_by = $request->get('group_by', 'supplier');
        if (! in_array($group_by, ['category', 'brand', 'supplier'])) {
            $group_by = 'supplier';
        }

        $period = (int) $request->get('period', 3);
        if (! in_array($period, [1, 3, 6, 9, 12])) {
            $period = 3;
        }
        ...
        $start = \Carbon::now()->subMonths($period - 1)->startOfMonth();
        ...
        // ----- Purchases: product x month -----
        $purchaseQuery = DB::table('purchase_lines as pl')
            ->join('transactions as t', 'pl.transaction_id', '=', 't.id')
            ...
            ->select(
                ...
                DB::raw('SUM(pl.quantity - pl.quantity_returned) as qty')
            )
            ->groupBy('p.id', 'c.id', DB::raw("DATE_FORMAT(t.transaction_date, '%Y-%m')"));

        // ----- Sales: product x month (status = final only) -----
        $salesQuery = DB::table('transaction_sell_lines as tsl')
            ...
            ->groupBy('p.id', DB::raw("DATE_FORMAT(t.transaction_date, '%Y-%m')"));

        if ($permitted_locations != 'all') {
            $purchaseQuery->whereIn('t.location_id', $permitted_locations);
            $salesQuery->whereIn('t.location_id', $permitted_locations);
        }
        if (! empty($location_id)) {
        ...
        // Entity filter restricts to a single category/brand/supplier.
        if (! empty($entity_id)) {
            if ($group_by == 'category') {
                $purchaseQuery->where('p.category_id', $entity_id);
                $salesQuery->where('p.category_id', $entity_id);
            } elseif ($group_by == 'brand') {
                $purchaseQuery->where('p.brand_id', $entity_id);
                $salesQuery->where('p.brand_id', $entity_id);
            } else {
                // Supplier is a property of the purchase only; sales are matched by product.
                $purchaseQuery->where('t.contact_id', $entity_id);
            }
        }
        ...
        // Sales indexed by product for supplier grouping (sales carry no supplier).
        $sales_by_product = [];
        foreach ($salesResult as $r) {
            $sales_by_product[$r->product_id][$r->ym] = ($sales_by_product[$r->product_id][$r->ym] ?? 0) + (float) $r->qty;
        }

        $uncategorized = __('lang_v1.uncategorized');
        $no_brand = __('lang_v1.no_brand');

        $groupKey = function ($row) use ($group_by, $uncategorized, $no_brand) {
            if ($group_by == 'category') {
                return [$row->category_id ?? 0, $row->category_name ?? $uncategorized];
            }
            if ($group_by == 'brand') {
                return [$row->brand_id ?? 0, $row->brand_name ?? $no_brand];
            }
            $name = ! empty($row->supplier_business_name) ? $row->supplier_business_name : $row->supplier_name;

            return [$row->supplier_id, $name];
        };

        $groups = [];

        foreach ($purchaseResult as $r) {
            [$gkey, $gname] = $groupKey($r);
            if (! isset($groups[$gkey])) {
                $groups[$gkey] = ['name' => $gname, 'products' => []];
            }
            if (! isset($groups[$gkey]['products'][$r->product_id])) {
                $groups[$gkey]['products'][$r->product_id] = [
                    'name' => $r->product_name,
                    'type' => $r->product_type,
                    'purchase' => [],
                    'sales' => [],
            ...
            $groups[$gkey]['products'][$r->product_id]['purchase'][$r->ym] =
                ($groups[$gkey]['products'][$r->product_id]['purchase'][$r->ym] ?? 0) + (float) $r->qty;
        }

        if ($group_by == 'supplier') {
            // Attach each product's sales (by product id) to every supplier row it appears in.
            foreach ($groups as $gkey => $group) {
                foreach ($group['products'] as $pid => $product) {
                    if (isset($sales_by_product[$pid])) {
                        $groups[$gkey]['products'][$pid]['sales'] = $sales_by_product[$pid];
                    }
                }
            }
        } else {
            // Category/brand: fold sales into their own group, surfacing sale-only products too.
            foreach ($salesResult as $r) {
                [$gkey, $gname] = $groupKey($r);
                if (! isset($groups[$gkey])) {
                    $groups[$gkey] = ['name' => $gname, 'products' => []];
                ...
                $groups[$gkey]['products'][$r->product_id]['sales'][$r->ym] =
                    ($groups[$gkey]['products'][$r->product_id]['sales'][$r->ym] ?? 0) + (float) $r->qty;
            }
        }
        ...
        foreach ($groups as $group) {
            $product_ids = array_merge($product_ids, array_keys($group['products']));
        }
        ...
        if (! empty($product_ids)) {
            $stockQuery = DB::table('variation_location_details as vld')
                ->whereIn('vld.product_id', $product_ids)
                ->select('vld.product_id', DB::raw('SUM(vld.qty_available) as stock'))
                ->groupBy('vld.product_id');

            if ($permitted_locations != 'all') {
                $stockQuery->whereIn('vld.location_id', $permitted_locations);
            }
            if (! empty($location_id)) {
                $stockQuery->where('vld.location_id', $location_id);
            }

            $stock = $stockQuery->pluck('stock', 'product_id')->all();
        }

        // Sort groups and their products alphabetically for a stable layout.
        uasort($groups, function ($a, $b) {
            return strcasecmp($a['name'], $b['name']);
        });
        foreach ($groups as $gkey => $group) {
            uasort($groups[$gkey]['products'], function ($a, $b) {
                return strcasecmp($a['name'], $b['name']);
            });
        }

        return view('report.partials.purchase_sale_product_data')
            ->with(compact('groups', 'months', 'stock', 'group_by'));
    }
