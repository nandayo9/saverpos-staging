...
class Contact extends Authenticatable
{
    ...
    public static function contactDropdown($business_id, $exclude_default = false, $prepend_none = true, $append_id = true)
    {
        ...
        //This list mixes suppliers and customers, so "view own" has to be applied per
        //type. The old check read the supplier permission only, so "view own supplier"
        //trimmed customers out of the list as well and "view all customer" had no effect
        //here - the full list only appeared once "view all supplier" was also granted.
        //A contact of type "both" is listed when either side allows it. Rows duplicated
        //by the join collapse again in the pluck below.
        $user = auth()->check() ? auth()->user() : null;
        $own_supplier = ! empty($user) && ! $user->can('supplier.view') && $user->can('supplier.view_own');
        $own_customer = ! empty($user) && ! $user->can('customer.view') && $user->can('customer.view_own');
        ...
        if ($own_supplier || $own_customer) {
            ...
            $query->where(function ($q) use ($user_id, $own_supplier, $own_customer) {
                $q->where(function ($q2) use ($user_id, $own_supplier) {
                    $q2->whereIn('contacts.type', ['supplier', 'both']);

                    if ($own_supplier) {
                        $q2->where(function ($q3) use ($user_id) {
                            $q3->where('contacts.created_by', $user_id)
                                ->orWhere('ucas.user_id', $user_id);
                        });
                    }
                })->orWhere(function ($q2) use ($user_id, $own_customer) {
                    $q2->whereIn('contacts.type', ['customer', 'both']);
