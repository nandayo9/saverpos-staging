...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'type' => $sale_type,
            'status' => $input['status'],
            // A transaction must have an initial payment state before payment
            // lines are written and the final status is recalculated below.
            // Older callers omit this field, while the schema requires it.
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'cheque_due_date' => ! empty($payment['cheque_due_date']) ? ($uf_data ? $this->uf_date($payment['cheque_due_date']) : $payment['cheque_due_date']) : null,
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        //Convert cheque due date from business format to db format (null when empty/non-cheque)
        if (isset($payment['cheque_due_date'])) {
            $payment['cheque_due_date'] = ! empty($payment['cheque_due_date'])
                ? ($uf_data ? $this->uf_date($payment['cheque_due_date']) : $payment['cheque_due_date'])
                : null;
        }
            ...
            if ($il->show_payments == 1) {
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        $paid_on = ! empty($il->common_settings['hide_payment_date']) ? '' : $this->format_date($value['paid_on'], false, $business_details);
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null, $permitted_locations = null)
    {
        ...
        $output['total_tax'] = $purchase_details->total_tax;
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
                    ...
                    ->select(
                        ...
                        DB::raw('SUM(shipping_charges) as total_shipping_charges'),
                        DB::raw('SUM(additional_expense_value_1 + additional_expense_value_2 + additional_expense_value_3 + additional_expense_value_4) as total_expense'),
                        DB::raw('SUM(tax_amount) as total_order_tax'),
                        DB::raw('SUM(COALESCE((SELECT SUM(tsl.item_tax * tsl.quantity) FROM transaction_sell_lines as tsl WHERE tsl.transaction_id = transactions.id), 0)) as total_inline_tax')
                    );
        ...
        //Inline tax (per line item) + transaction-level tax
        $output['total_tax'] = $sell_details->total_inline_tax + $sell_details->total_order_tax;

        return $output;
    }

    /**
     * Returns a base TransactionSellLine query joined to transactions with the common
     * sell-line filters applied (business, type=sell, status=final, date range,
     * permitted locations, location, customer, customer group, created_by).
     *
     * Callers add their own joins (products, variations, categories, brands, etc.)
     * and select/groupBy/orderBy/limit as needed.
     *
     * Used by product-sell reports and CustomDashboard widgets that aggregate sell lines.
     *
     * @param  int  $business_id
     * @param  array  $filters Optional keys: start_date, end_date, location_id,
     *                        permitted_locations, customer_id, customer_group_id, created_by
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function getSellLinesQuery($business_id, $filters = [])
    {
        ...
        if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
            $query->where('t.transaction_date', '>=', $filters['start_date'])
                  ->where('t.transaction_date', '<=', $filters['end_date']);
        }

        $permitted_locations = array_key_exists('permitted_locations', $filters)
            ? $filters['permitted_locations']
        ...
        if (!empty($filters['customer_id'])) {
            $query->where('t.contact_id', $filters['customer_id']);
        }

        if (!empty($filters['customer_group_id'])) {
            $query->leftjoin('contacts AS c', 't.contact_id', '=', 'c.id')
                  ->leftjoin('customer_groups AS CG', 'c.customer_group_id', '=', 'CG.id')
                  ->where('CG.id', $filters['customer_group_id']);
        }

        if (!empty($filters['created_by'])) {
            $query->where('t.created_by', $filters['created_by']);
        }
    ...
    /**
     * Total line-level (inline) discount given on sells for the period.
     * Sum of (unit_price_before_discount - unit_price) * quantity over sell lines.
     */
    public function getTotalSellLineDiscount($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        //Apply the date filter here with whereDate. getSellLinesQuery compares the datetime column
        //directly, which drops same-day transactions that have a time component (e.g. 23:43).
        $query = $this->getSellLinesQuery($business_id, [
        ...
        $result = $query->select(
            DB::raw('SUM((transaction_sell_lines.unit_price_before_discount - transaction_sell_lines.unit_price) * transaction_sell_lines.quantity) as total_line_discount')
        )->first();

        return ! empty($result->total_line_discount) ? $result->total_line_discount : 0;
    }

    /**
     * Total line-level (inline) discount given on purchases for the period.
     * Sum of (pp_without_discount - purchase_price) * quantity over purchase lines.
     */
    public function getTotalPurchaseLineDiscount($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = PurchaseLine::join('transactions as t', 'purchase_lines.transaction_id', '=', 't.id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'purchase')
            ->select(
                DB::raw('SUM((purchase_lines.pp_without_discount - purchase_lines.purchase_price) * purchase_lines.quantity) as total_line_discount')
            );
    ...
    public function purchaseCurrencyDetails($business_id)
    {
        ...
        // ProductUtil's shared purchase-line formatter accepts the same
        // business-detail shape used by ordinary purchase pages. Include the
        // precision and symbol fields here so API-style receiving flows do
        // not depend on a browser currency session being present.
        $output['currency_precision'] = (int) ($business->currency_precision ?? 2);
        $output['quantity_precision'] = (int) ($business->quantity_precision ?? 2);
        $output['currency_symbol_placement'] = $business->currency_symbol_placement ?? 'before';
        $output['currency_symbol'] = $currency->symbol;
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            foreach ($sell_purchases as $line) {
                if (empty($line->slpl_id)) {
                    ...
                    //NOTE: dedup must use $line->id (sell_line id), NOT $line->slpl_id (TSP row id).
                    //The LEFT JOIN returns one row per (sell_line × TSP) pair, so a sell line with
                    //multiple TSP mappings appears multiple times. Using slpl_id never deduped because
                    //each TSP row has a unique id — caused duplicate entries in $new_sell_lines on
                    //quantity-increase edits, leading to over-allocation in mapPurchaseSell.
    ...
    /**
     * Decrease quantity_sold on purchase lines and remove/shrink the corresponding
     * transaction_sell_lines_purchase_lines (junction) rows when a sell line is
     * deleted or its quantity is reduced.
     *
     * Processes junction rows newest-first (id DESC) so the most recently
     * allocated stock is released first (LIFO release order).
     *
     * Called from two places inside adjustMappingPurchaseSell():
     *   1. When a sell line is fully deleted from a final transaction —
     *      called once per junction row, passing that row's quantity.
     *   2. When a sell line's quantity is reduced on a final transaction —
     *      called once with the total difference (old mapped qty - new qty).
     *
     * @param  int        $sell_line_id   ID from transaction_sell_lines
     * @param  float|int  $decrement_qty  Total quantity to un-allocate
     * @return void
     */
    private function mapDecrementPurchaseQuantity($sell_line_id, $decrement_qty)
    {
        // Load all junction rows for this sell line, newest allocation first.
        ...
        foreach ($sell_purchase_line as $row) {
            if ($row->quantity > $decrement_qty) {
                // This row holds MORE than what still needs to be removed.
                // Partially reduce the row and stop — no need to touch the next row.
                PurchaseLine::where('id', $row->purchase_line_id)
                ...
                // Set to 0 so the post-loop calculation ($decrement_qty - $row->quantity)
                // produces a negative value and the break fires correctly.
                $decrement_qty = 0;
            } else {
                // This row holds LESS THAN OR EQUAL to what still needs to be removed.
                // Remove the entire row and continue to the next row for the remainder.
                //
                // IMPORTANT: decrement by $row->quantity (what this row actually held),
                // NOT by $decrement_qty (the remaining budget which can be larger).
                // Using $decrement_qty here would over-decrement quantity_sold and push
                // it negative, making that purchase batch appear to have phantom stock
                // on the next FIFO allocation query.
                PurchaseLine::where('id', $row->purchase_line_id)
                    ->decrement('quantity_sold', $row->quantity);
            ...
            // Reduce the remaining amount to remove.
            // In the if-branch  : $decrement_qty was set to 0 above, so this becomes
            //                     negative → triggers break below.
            // In the else-branch: subtracts what this row held → carries the remainder
            //                     forward to the next junction row.
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (! empty($transaction)) {
            app(\App\Services\WalkInService::class)->releaseConversionForTransaction($transaction, auth()->user());
            $log_properities = [
                'id' => $transaction->id,
                'invoice_no' => $transaction->invoice_no,
            ];
     ...
     * @return object
     */
    public function getListSells($business_id, $sale_type = 'sell', $only_count = false)
    {
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $prev_total_invoice_paid = $prev_payments->whereIn('transaction_type', ['sell', 'hms_booking', 'gym_subscription'])->where('is_return', 0)->sum('amount');

        $prev_total_ob_paid = $prev_payments->where('transaction_type', 'opening_balance')->where('is_return', 0)->sum('amount');
        $prev_total_sell_change_return = $prev_payments->whereIn('transaction_type', ['sell', 'hms_booking', 'gym_subscription'])->where('is_return', 1)->sum('amount');
        ...
        foreach ($transactions as $transaction) {
            ...
            $temp_array = [
                ...
                'total' => '',
                'payment_method' => '',
                'debit' => in_array($transaction->type, ['sell', 'purchase_return', 'hms_booking', 'gym_subscription']) || ($transaction->sub_type == 'purchase_discount') ? $transaction->final_total : '',
        ...
        $hms_booking_sum = $transactions->where('type', 'hms_booking')->sum('final_total');
        $gym_subscription_sum = $transactions->where('type', 'gym_subscription')->sum('final_total');
        ...
        foreach ($payments as $payment) {
            ...
            $ledger[] = [
                ...
                'credit' => (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance','hms_booking', 'gym_subscription']) || ($payment->is_advance == 1 && in_array($contact->type, ['customer', 'both']))) && $payment->is_return == 0 || $payment->payment_type == 'credit' ? $payment->amount : '',
                'others' => $note,
            ];
        }
        ...
        $total_invoice_paid = ! empty($payments) ? $payments->whereIn('transaction_type', ['sell', 'hms_booking', 'gym_subscription'])->where('is_return', 0)->sum('amount') : 0;

        $total_sell_change_return = ! empty($payments) ? $payments->whereIn('transaction_type', ['sell', 'hms_booking', 'gym_subscription'])->where('is_return', 1)->sum('amount') : 0;
        ...
        $overall_payments = $this->__paymentQuery($contact_id, null, null, $location_id)
                            ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 'is_advance')
                                    ->get();
        $overall_total_invoice_paid = $overall_payments->whereIn('transaction_type', ['sell', 'hms_booking', 'gym_subscription'])->where('is_return', 0)->sum('amount');
        $overall_total_ob_paid = $overall_payments->where('transaction_type', 'opening_balance')->where('is_return', 0)->sum('amount');
        $overall_total_sell_change_return = $overall_payments->whereIn('transaction_type', ['sell', 'hms_booking', 'gym_subscription'])->where('is_return', 1)->sum('amount');
    ...
    private function __paymentQuery($contact_id, $start, $end = null, $location_id = null)
    {
        ...
        $query = TransactionPayment::leftJoin(
            ...
            //$business_id was read above but never applied, so the only thing scoping this
            //query was payment_for. Rows belonging to another business are excluded here.
            //business_id is nullable and was backfilled from the payment's creator, so
            //older rows can still hold null - those are kept rather than silently dropped
            //from the ledger, since a null cannot be attributed to another business.
            ->where(function ($query) use ($business_id) {
                $query->where('transaction_payments.business_id', $business_id)
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        //Tax summary (using actual tax_amount column)
        ...
        $expense_by_category_query = Transaction::where('transactions.business_id', $business_id)
            ->whereIn('transactions.type', ['expense', 'expense_refund'])
            ->leftJoin('expense_categories as ec', 'transactions.expense_category_id', '=', 'ec.id');

        if (! empty($start_date) && ! empty($end_date)) {
            $expense_by_category_query->whereDate('transactions.transaction_date', '>=', $start_date)
                ->whereDate('transactions.transaction_date', '<=', $end_date);
        }
        if (! empty($location_id)) {
            $expense_by_category_query->where('transactions.location_id', $location_id);
        }
        if (! empty($user_id)) {
            $expense_by_category_query->where('transactions.created_by', $user_id);
        }
        if (! empty($permitted_locations) && $permitted_locations != 'all') {
            $expense_by_category_query->whereIn('transactions.location_id', $permitted_locations);
        }

        $data['expenses_by_category'] = $expense_by_category_query
            ->select(
                DB::raw("COALESCE(ec.name, '" . __('lang_v1.uncategorized') . "') as category_name"),
                DB::raw("SUM(IF(transactions.type='expense', final_total, 0)) - SUM(IF(transactions.type='expense_refund', final_total, 0)) as category_total")
            )
            ->groupBy('transactions.expense_category_id')
            ->havingRaw('category_total != 0')
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        $transaction_data = $request->only(['ref_no', 'transaction_date',
            ...
            'expense_category_id', 'tax_id', 'contact_id', 'expense_title', 'payment_to', ]);
    ...
    public function updateExpense($request, $id, $business_id, $format_data = true)
    {
        ...
        if ($request->has('expense_title')) {
            $transaction_data['expense_title'] = $request->input('expense_title');
        }
        if ($request->has('payment_to')) {
            $transaction_data['payment_to'] = $request->input('payment_to');
        }
