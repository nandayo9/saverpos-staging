...
class ContactUtil extends Util
{
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        ...
        if ($type == 'supplier') {
            ...
            //A mixed list, so "view own" has to be applied per type: "view own supplier"
            //must not hide customers and vice versa.
            //The old guard read `check() && (A) || (B)`, and && binds tighter than ||, so
            //the supplier half sat outside the auth()->check() guard and called can() on
            //a null user whenever this ran without a session.
