...
Current milestone: Recommerce — staging Cash flow verified; the served deployment is blocked in the cPanel repository-update step
...
Next safe task: confirm the cPanel document-root choice, then run the approved cPanel Update from Remote → Deploy HEAD step manually. Workflow run `33300239447` failed in the new repository-update request with exit code 5 before deployment. The browser inspection recorded the domain root as `.../saverpos-staging-repo/public`, while `scripts/cpanel-staging-deploy.sh` defaults to publishing into the sibling `.../saverpos-staging/public`; do not change that path without the operator confirming which directory actually serves the domain. Verify a served asset matches the published checkout, then repeat the focused staging smoke against that deployed fixture. Do not advance RC-045/RC-046 from this flow evidence alone.
...
Hosting prep: iCore cPanel has PHP 8.2, MySQL, and Let's Encrypt SSL. The browser inspection recorded the domain root as `/home/kkcctv93/repositories/saverpos-staging-repo/public`, but the deployment script defaults to `/home/kkcctv93/repositories/saverpos-staging/public`; this host-path mismatch needs operator confirmation before changing the deployment target. Workflow run `33300239447` for `61fbc7f` failed with exit code 5 in the cPanel repository-update step, so deployment did not run. The served `public/css/saverbro-dark-pos.css` hash remains `3c2ab4f7…` while the local published checkout is `798306ec…`; the browser verifies the site is reachable and the staging flow above is live evidence, not proof of the current checkout. No cPanel credentials or database password are present in this checkout.
