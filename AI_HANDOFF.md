...
Current status update (2026-08-31): RC-041 archive writes and read-only inspection are committed and covered by focused/full tests. The checkout still has no licensed provider Repair source or legacy Repair tables, so full historical completeness remains unverified. RC-038 trade-in acquisition is implemented and deployed to the fictional staging estate at commit `1459d68`: deterministic versioned pricing, structured evidence, approval gates, one-unit native purchase seam, append-only acquisition/reversal history, and a server-rendered UI. Full suite is green at 363 tests / 1,797 assertions. The public `/recommerce/trade-ins` endpoint now returns the expected authenticated redirect (HTTP 302 to `/login`) instead of 404. Browser payment was previously proven on an isolated clone; browser reversal remains unverified because no reversal UI exists.
...
Latest implementation commits: **Land RC-038 trade-in**, **Prove RC-038 settlement**, and **Enforce the authorization gate structurally** are published on `origin/staging` at `1459d68`. The cPanel poller has pulled the branch and the live route responds. The earlier diagnostics, repair-queue, and deployment commits remain historical evidence.
...
Next safe task: the board is now decision-bound. RC-038's last gap is the reversal UI; RC-033's quote UI, the authenticated-422 disclosure convention, RC-022, RC-043 and the duplicate `logout` route all need an operator call before code. Nothing further is safely buildable without one. Alternatively obtain product decisions for the uncalled RC-033 quote UI and authenticated-422 disclosure convention. Do not re-deploy or change staging data without an explicit release instruction. Do not bypass, disable globally, or automate the JavaScript anti-bot challenge. Do not advance RC-045/RC-046 from this flow evidence alone.
...
RCR-010 decision 2 defers settlement to native UltimatePOS payment flows, with ownership transferring when the purchase
posts even while it is still `due`. That was a claim; it is now evidence.

On the isolated `saverpos_demo_rc038` clone, the accepted trade-in's purchase `PO2026/0001` (RM 950.00, `received` /
`due`) was paid through **Purchases → Actions → Add payment** — the native `TransactionPaymentController`, no
Recommerce code involved. UltimatePOS wrote one `transaction_payments` row (RM 950.0000, `cash`) and flipped
`payment_status` to `paid`. Every trade-in-scoped Recommerce row — device, 2 ownership periods, 3 custody periods,
2 movements, the acquisition, the valuation — came back **byte-identical**, `lock_version` included.

Worth noticing while the modal was open: the native payment note carries `Trade-in valuation <uuid>`, which is the
RCR's requirement that the native record reference the offer without putting device data in free text.

### Two traps I walked into, recorded so you don't

1. **A shell loop that never looped.** My first whole-table before/after diff printed "UNCHANGED" for everything —
   because zsh does not word-split an unquoted `$TABLES`, so all seven tables were hashed as a single blob in one
   iteration. Use an array (`tables=(...)`, `"${tables[@]}"`).
2. **An "unchanged" that would also fire on nothing at all.** Two empty dumps compare equal. The corrected check counts
   rows on both sides and reports `NO ROWS - not evidence` rather than a green tick. It then showed several tables
   genuinely changed — because **this session had run `SaverposDemoExpansionSeeder` against the fixture** between the
   snapshot and the payment. The published comparison is therefore scoped to the trade-in device, which isolates
   settlement from that noise. If you snapshot before a proof, do not seed between the snapshot and the measurement.

### A small re-verification that fell out

The fixture's two locations had `default_payment_accounts` of `NULL`, so the native payment modal offered only
"Advance" — the same fixture gap already recorded for the POS register. `SaverposDemoExpansionSeeder` repaired it, which
re-verifies that earlier fix in a fresh database. The clone is deliberately left settled: a future reversal proof starts
from a **paid** purchase, not a due one.
...
1. ~~Get a signed-in session and do an interaction pass.~~ **DONE** — the staging smoke walked receive → transfer → sale → return → reconciliation.
...
Every mutation in the repair flow was driven signed in against `saverpos_demo_p0`. **No application defect was found.**
One decision is owed, and one harness trap cost me time and nearly became a false bug report.

### What ran

| Path | Result |
| --- | --- |
| Transition, refused | `AWAITING_APPROVAL → WAITING_PARTS` with no evidence was rejected and the result box rendered |
| Transition, accepted | With `{"approval_satisfied":true}` the job moved, `lock_version` 3 → 4, the evidence persisted verbatim as audit, and the page reloaded |
| Intake | The real `RepairJobIntakeService` — through the real cohort/permission gate — created a job in `RECEIVED` with a customer-owned device (`CUSTOMER`/`NONE`/`RECEIVED`), all six checklist outcomes, a `command_hash`, and one active lookup token |
| Public tracking | The issued raw token opened the customer status page, disclosing only code, category/brand/model, state, and the customer-facing update |
| Part reservation | A `RESERVED` row was written and `variation_location_details.qty_available` **stayed at 1.0000** — the "reservations do not mutate POS stock" boundary holds — after which the form correctly reported no available stock |

The estate was restored from a pre-pass dump, so the demo queue is back to its reproducible four-job spread. Note for
anyone dumping these databases: `mysqldump` here emits a `SET @@GLOBAL.GTID_PURGED` line that makes the dump
un-restorable onto the same server; strip it, or dump with `--set-gtid-purged=OFF`.

**A useful cross-check fell out of this.** The job the real intake service produced matches the shape
`SaverposDemoRepairFixture` writes, differing only where intended: the service records a `command_hash` derived from
the submitted payload, the fixture leaves it null because there was no payload. The fixture is not drifting from the
service it mirrors.

### The decision: transition errors throw away the guidance the domain wrote

`RepairJobController::transition` catches `ValidationException|LogicException` and replaces **every** message with
`'Repair transition was rejected.'`. The state machine and quote service author precise, non-sensitive guidance that
never reaches the operator: *"Work cannot begin before approval policy is satisfied."*, *"READY requires an explicit
resolution code."*, *"QC rework requires a failure reason."*, *"Approve the current quote version before starting
repair work."*

The one that matters most is the concurrency message — `Repair job changed; reload before retrying.` A user holding a
stale `expected_lock_version` is told only that the transition was rejected, so the obvious response is to resubmit the
same stale form, which can never succeed.

**Not changed here, deliberately.** The module holds two conventions: `RepairJobController::intake`, in the same file,
returns `$exception->getMessage()` verbatim, while `DiagnosticController` uses the generic form in both actions.
`RECOMMERCE_SECURITY_AND_PERMISSIONS.md:127` forbids putting *sensitive values* into exception messages — these carry
no values at all, and the endpoint sits behind `recommerce.repair.transition` and the cohort gate — but choosing which
convention governs authenticated 422s is a disclosure decision, not a bug fix. Whoever settles it should also decide
whether `ValidationException` keeps the generic response while `LogicException` surfaces its message.

### The harness trap — read this before reporting a broken button

Two button clicks appeared to do nothing: no network request, no console error, no result box. That looks exactly like
a dead submit handler, and I started writing it up as one. It was not.

`computer{action:"left_click", ref:"..."}` reports coordinates in the **full-resolution** frame (1440x900 here) while
`computer{action:"screenshot"}` returns an 800x500 image, and manual coordinates are rejected unless they fall inside
that 800x500 frame. The ref-based clicks were landing off-target. Clicking with coordinates read off the screenshot
worked every time, and `form.requestSubmit()` / `dispatchEvent(new Event('submit', {cancelable:true}))` proved the
handlers were attached and functioning all along — `dispatchEvent` returning `false` is a clean test for "something
called preventDefault".

**Before concluding a control is broken, prove the event never fired.** A silent no-op is more often the driver missing
the element than the page failing to handle it.
...
The previous handoff recorded "0 repair jobs" as a finding and listed the fix as suggested item 2. Closed here.

Without any repair job the queue, the repair record, and everything hanging off it — quote, parts, billing,
collection — could not be opened with real data at all. `Database\Seeders\SaverposDemoRepairFixture` now opens four
fictional CUSTOMER_REPAIR jobs, one per demo customer, at Branch A, in RECEIVED / DIAGNOSIS / AWAITING_APPROVAL /
IN_REPAIR, each with its customer-owned device, the configured intake checklist, its transition history, and an
active public lookup token.

### Three decisions worth knowing before changing it

**Customer repairs only, on purpose.** A customer-owned device carries `stock_participation = NONE`, so the fixture
cannot move core stock or the tracked reconciliation counts the staging smoke is verified against. An internal
refurbishment job would have had to consume one of the cohort-variation units the receive/transfer/sale flows already
use. Measured rather than assumed: 17 business-owned `ON_HAND` devices against 17 `qty_available` at Branch A, identical
before and after.

**Rows are written directly, and the gate is not touched.** Using `RepairJobIntakeService` would have meant the seeder
switching `recommerce.enabled`/`writes_enabled` and the cohort IDs on for itself and impersonating the demo admin — a
seeder granting itself the authorization gate. Given RC-041 is blocked for precisely that class of defect, that is not
a call to make silently. A seeder is a data fixture, not an actor, and the surrounding demo seeders already write
devices, movements, and periods directly. Everything borrowable from the module *is* borrowed:
`RepairJobStateMachine::assertNewJob` asserts each job is legally formed, the real `RepairJobTransitionService`
performs every state advance (it carries no authorization of its own — the controller does), checklist labels come
from config rather than a copy, and device codes come from `DeviceCode::forDeviceId`.

**It had to go into both seeders.** This is the payment-account lesson, and it would have been repeated otherwise:
`SaverposDemoRuntimeSeeder` only runs against a *fresh* database, so a fixture added there alone never arrives on the
already-seeded staging estate. `SaverposDemoExpansionSeeder` is what reaches it. Both call one shared fixture class so
they cannot drift.
...
| Check | Result |
| --- | --- |
| Fresh seed on disposable MySQL | `repair_jobs=4`; four jobs, four states, 24 checklist items, 4 active lookup tokens, transition chains `RECEIVED → … → <state>` |
| Re-run | `repair_jobs=0`, nothing changed — idempotent on a deterministic UUIDv5 `command_uuid` per job |
| Expansion seeder against an estate stripped of repair rows | `repair_jobs=4` — the deployed-estate path works |
| Stock | 17 `BUSINESS`/`ON_HAND` devices, 17 `qty_available` at Branch A, untouched |
| Suite | **314 tests / 1451 assertions green**, PHP 8.2.33; `recommerce-static-check` passes |
| Mutation check | 8 mutations of the fixture (stock participation, temp device code, hardcoded checklist label, dropped idempotency, no state advance, throwing on a missing customer, no lookup token, ignoring soft deletes) — **8 caught** |

The disposable database was dropped afterwards; the three existing `saverpos_demo_*` databases were not touched.

### What this is not

Nobody has looked at the seeded queue in a browser. The screens were render-tested earlier against stub data, not
against these rows, and no repair interaction has been exercised. That is the obvious next step, and it now has data
to work with. Staging has no repair jobs until this commit is pushed and the Cron deploy runs the expansion seeder.
