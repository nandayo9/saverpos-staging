...
Current milestone: Recommerce — staging Cash flow verified; server-local cPanel Cron deployment selected to avoid the protected external API
...
Next safe task: make the one-time cPanel **Update from Remote**, add the `scripts/cpanel-staging-poll.sh` Cron Job, and perform one initial deploy. The Cron Job fetches and fast-forward merges `origin/staging` from inside the cPanel account, then invokes the existing deployment script only when the branch advances. Do not bypass, disable globally, or automate the JavaScript anti-bot challenge. After setup, verify a Cron-driven deployment and a served asset matching the published checkout. Do not advance RC-045/RC-046 from this flow evidence alone.
...
Hosting prep: iCore cPanel has PHP 8.2, MySQL, and Let's Encrypt SSL. Browser inspection confirmed the managed repository path `/home/kkcctv93/repositories/saverpos-staging-repo`; a manual **Update from Remote** successfully advanced it to `37be8b3`. cPanel keeps deployment disabled until its checkout is clean, and the root runtime `error_log` is now ignored in Git. GitHub API-triggered deployment remains unusable: workflow run `33300932113` (four attempts) and a read-only local request receive an HTTP 200 JavaScript "One moment, please…" anti-bot page rather than UAPI JSON. The selected no-support alternative is the server-local `scripts/cpanel-staging-poll.sh` Cron Job, which polls the public `staging` branch internally with a lock and fast-forward-only guard. The served `public/css/saverbro-dark-pos.css` hash remains `3c2ab4f7…` while the local published checkout is `798306ec…`; the dark UI is not live until the one-time Cron setup and initial deploy are verified. No cPanel credentials or database password are present in this checkout.
