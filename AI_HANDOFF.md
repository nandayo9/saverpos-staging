...
Current milestone: Recommerce — staging Cash flow verified; GitHub-to-cPanel auto-deployment is blocked by the cPanel API anti-bot boundary
...
Latest implementation commits: `71cb6fd` ignores the cPanel-generated root `error_log`; `c1679b4` normalizes the cPanel host and fails safely on non-JSON responses. Both are pushed to `staging`. The working tree is dirty by design: the untracked files and the two modified shared files are the blocked RC-041 archive and must not be committed.
...
Next safe task: obtain an approved, unchallenged authenticated cPanel UAPI endpoint from the hosting provider (or a narrowly scoped WAF exception for cPanel `/execute` on port 2083), then set `CPANEL_HOST` to that endpoint and rerun the workflow. Do not bypass, disable globally, or automate the JavaScript anti-bot challenge. After that, verify one successful GitHub Actions pull-and-deploy run and a served asset matching the published checkout. Do not advance RC-045/RC-046 from this flow evidence alone.
...
Hosting prep: iCore cPanel has PHP 8.2, MySQL, and Let's Encrypt SSL. Browser inspection confirmed the managed repository path `/home/kkcctv93/repositories/saverpos-staging-repo`; a manual **Update from Remote** successfully advanced it to `37be8b3`. cPanel keeps deployment disabled until its checkout is clean, and the root runtime `error_log` is now ignored in Git. GitHub secrets were refreshed through the browser, but workflow run `33300932113` (four attempts) still stops at `VersionControl/update`: both its authenticated request and a read-only local request return HTTP 200 `text/html` with a JavaScript "One moment, please…" anti-bot challenge, not UAPI JSON. This is a hosting security boundary, not a credential or Git defect. The served `public/css/saverbro-dark-pos.css` hash remains `3c2ab4f7…` while the local published checkout is `798306ec…`; the dark UI is not live. No cPanel credentials or database password are present in this checkout.
