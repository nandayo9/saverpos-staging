...
Hosting prep: iCore cPanel now has `pos.kkcctv.com.my` mapped to the cloned `staging` repository's `public/` directory, PHP 8.2 enabled account-wide, staging MySQL database/user created, and Let's Encrypt SSL installed. cPanel Git pull is verified at commit `bd8f49f`. cPanel shell/Terminal access is disabled and the repository has no `.cpanel.yml`, so server `.env` creation, Composer install, migrations, and demo seeders remain pending iCore enabling SSH/Terminal or approving a deployment-task configuration; no cPanel credentials or database password are present in this checkout.
