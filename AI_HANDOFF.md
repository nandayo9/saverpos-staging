...
Hosting prep: iCore cPanel staging guide targets `https://pos.kkcctv.com.my`; local `staging` branch is connected to `https://github.com/nandayo9/saverpos-staging.git` and the remote push is verified at `4a0169c`. cPanel/DNS deployment remains pending hosting-account access. Use a new MySQL database and PHP 8.2; no cPanel credentials are present in this checkout.
