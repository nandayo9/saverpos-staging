...
Last completed task: **Configure the server-local staging deploy Cron Job** — cPanel accepted the exact five-minute `scripts/cpanel-staging-poll.sh` command on 2026-08-30 and invoked it at 16:35. Its managed checkout is still on a pre-helper commit, so one recovery update is required before the first Cron-driven deployment.
...
Next safe task: make one cPanel **Update from Remote** to move the managed checkout from `37be8b3` to the already-pushed helper, then wait for the next five-minute interval and verify the Cron log and served dark stylesheet. After that one bootstrap update, normal deployments are GitHub pushes only: the Cron Job fetches and fast-forward merges `origin/staging`, then invokes the existing deployment script only when the branch advances. Do not bypass, disable globally, or automate the JavaScript anti-bot challenge. Do not advance RC-045/RC-046 from this flow evidence alone.
...
Hosting prep: iCore cPanel has PHP 8.2, MySQL, and Let's Encrypt SSL. Browser inspection confirmed the managed repository path `/home/kkcctv93/repositories/saverpos-staging-repo`; its actual HEAD is `37be8b3`, a commit before `scripts/cpanel-staging-poll.sh` was added. cPanel keeps its GUI deployment action disabled while it sees a generic dirty-check failure, so it is not the normal path. cPanel now confirms the server-local Cron entry `*/5 * * * * /bin/bash /home/kkcctv93/repositories/saverpos-staging-repo/scripts/cpanel-staging-poll.sh >> /home/kkcctv93/repositories/saverpos-staging-repo/storage/logs/cpanel-staging-cron.log 2>&1`; its 16:35 log proves scheduling runs, but the helper cannot exist until one **Update from Remote** loads the pushed `263f81b` or later checkout. GitHub API-triggered deployment remains unusable: workflow run `33300932113` (four attempts) and a read-only local request receive an HTTP 200 JavaScript "One moment, please…" anti-bot page rather than UAPI JSON. After bootstrap, the Cron helper polls the public `staging` branch internally with a lock and fast-forward-only guard. The live dashboard is visibly dark and loads `saverbro-dark-pos.css`, but a Cron-driven deployment is still unverified. No cPanel credentials or database password are present in this checkout.
