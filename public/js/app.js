$(document).ready(function () {
    ...
    var units_table = $('#unit_table').DataTable({
        ...
        columns: [
            { data: 'actual_name', name: 'actual_name', className: 'text-center' },
            { data: 'short_name', name: 'short_name', className: 'text-center' },
            { data: 'allow_decimal', name: 'allow_decimal', className: 'text-center' },
            { data: 'action', name: 'action', className: 'text-center sb-col-fit' },
        ],
    });
    ...
    if (contact_table_type == 'supplier') {
        var columns = [
            // Codes, numbers, dates and money read better centred; names and
            // addresses stay left aligned so they scan as text.
            { data: 'action', searchable: false, orderable: false },
            { data: 'contact_id', name: 'contact_id', className: 'text-center' },
            { data: 'supplier_business_name', name: 'supplier_business_name' },
            { data: 'name', name: 'name' },
            { data: 'email', name: 'email' },
            { data: 'tax_number', name: 'tax_number', className: 'text-center' },
            {
                data: 'pay_term',
                name: 'pay_term',
                searchable: false,
            ...
            { data: 'balance', name: 'balance', searchable: false, className: 'text-center' },
            { data: 'created_at', name: 'contacts.created_at', className: 'text-center' },
            { data: 'address', name: 'address', orderable: false },
            { data: 'mobile', name: 'mobile', className: 'text-center' },
            { data: 'due', searchable: false, orderable: false, className: 'text-center' },
            { data: 'return_due', searchable: false, orderable: false, className: 'text-center' },
        ...
        // Same treatment as the supplier table: codes, numbers, dates and money
        // centred, names and addresses left aligned.
        var columns = [
            { data: 'action', searchable: false, orderable: false },
            { data: 'contact_id', name: 'contact_id', className: 'text-center' },
            { data: 'supplier_business_name', name: 'supplier_business_name' },
            ...
            { data: 'email', name: 'email' },
            { data: 'tax_number', name: 'tax_number', className: 'text-center' },
            { data: 'credit_limit', name: 'credit_limit', className: 'text-center' },
            {
                data: 'pay_term',
                name: 'pay_term',
                searchable: false,
    ...
    #has_purchase_due, #has_purchase_return, #has_advance_balance, #has_opening_balance',
        function () {
    ...
    var variation_table = $('#variation_table').DataTable({
        ...
        columnDefs: [
            ...
            {
                // Values has to sit on one line and size itself to its
                // longest entry. sb-col-fit is width:1% plus nowrap, and
                // table-colresize.js measures the widest cell's scrollWidth
                // to set the real width - that measurement is uncapped,
                // unlike the ordinary content path, which stops at
                // MAX_CONTENT_WIDTH (320px) and was what forced the wrap.
                targets: 1,
                className: 'text-left sb-col-fit',
            },
            {
                targets: 2,
                orderable: false,
    ...
    var tax_groups_table = $('#tax_groups_table').DataTable({
        ...
        columns: [
            ...
            { data: 'sub_taxes', name: 'sub_taxes', className: 'text-center' },
            { data: 'action', name: 'action', className: 'text-center sb-col-fit' },
        ],
    });
    ...
    business_locations = $('#business_location_table').DataTable({
        ...
        // Let DataTables own the horizontal scroll. The markup used to leave
        // that to a .table-responsive wrapper, but DataTables builds its own
        // wrapper inside it, so the search box, the info line and the pager
        // all sat in the scrolling box and slid away with the columns.
        // scrollX scrolls only the table body and leaves that chrome put.
        scrollX: true,
        scrollCollapse: true,
        columnDefs: [
            {
            ...
            {
                // Everything but Name and Landmark, which are free text and
                // read better ranged left.
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        columns: [
            ...
            { data: 'expense_title', name: 'transactions.expense_title' },
            { data: 'payment_to', name: 'transactions.payment_to' },
            ...
            { data: 'tax', name: 'tr.name', className: 'text-center' },
            ...
            { data: 'added_by', name: 'usr.first_name', className: 'text-center' },
        ],
    ...
    var sales_commission_agent_table = $('#sales_commission_agent_table').DataTable({
        ...
        columns: [
            ...
            { data: 'contact_no' },
            { data: 'address' },
            // A short numeric value; reads better centred than ranged left.
            // Vertical centring comes from the shared table rules.
            { data: 'cmmsn_percent', className: 'text-center' },
            { data: 'action' },
        ],
    });
    ...
    var customer_groups_table = $('#customer_groups_table').DataTable({
        ...
        columnDefs: [
            ...
            {
                // Calculation Percentage and Selling Price Group: short,
                // column-like values that read better centred than ranged
                // left. Vertical centring comes from the shared table rules.
                targets: [1, 2],
                className: 'text-center',
            },
        ],
...
//The layout is viewport locked, so the content scrolls inside #scrollable-container
//instead of the window. Fall back to the window where that container is absent.
$(function () {
    var $scroll_area = $('#scrollable-container').length ? $('#scrollable-container') : $(window);
    ...
    $('.scroll').click(function () {
        if ($scroll_area.is(window)) {
            $('html,body').animate({ scrollTop: $('.thetop').offset().top }, '1000');
        } else {
            $scroll_area.animate({ scrollTop: 0 }, '1000');
        }
