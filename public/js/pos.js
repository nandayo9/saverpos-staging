...
$(document).ready(function() {
    ...
    if ($('form#edit_pos_sell_form').length > 0) {
        ...
        setTimeout(function(){ orig_pos_edit_data = pos_form_obj.serialize(); }, 1000);
    } else {
...
// on click sub category in category drawer
    ...
    window.onbeforeunload = function() {
        if($('form#edit_pos_sell_form').length == 0){
            ...
            if (orig_pos_edit_data !== null && pos_form_obj.serialize() != orig_pos_edit_data) {
                return LANG.sure;
            }
        }
    }
    ...
    if (location_id) {
        $.ajax({
            ...
            success: function(result) {
                ...
                update_featured_products_count($box.find('.product_box').length);
            },
            error: function() {
                $box.html(emptyHtml);
                update_featured_products_count(0);
            },
        });
    } else {
        $box.html(emptyHtml);
...
// Keeps the count badge on the Featured Products button in sync with the
// currently selected location (the badge is server-rendered only for the
// default location at page load).
function update_featured_products_count(count) {
    var $badge = $('#featured_products_count');
    if ($badge.length === 0) {
        return;
    }
    $badge.text(count);
    if (count > 0) {
        $badge.removeClass('tw-hidden');
    } else {
        $badge.addClass('tw-hidden');
    }
}
...
// Initialize cheque due-date pickers. Idempotent: skips any field that is
// already initialized, so calling it from multiple places never stacks
// duplicate calendars on the same input.
function __init_date_pickers(scope) {
    var $scope = scope ? $(scope) : $(document);
    $scope.find('.date_picker').each(function () {
...
// Direct sell create/edit renders payment rows inline (not inside #modal_payment),
// so their date pickers must be initialized on page load.
$(document).ready(function () {
    __init_date_pickers();
});
...
$(document).on('change', '.payment_types_dropdown', function(e) {
    ...
    __init_date_pickers(payment_row);
});
