$(document).ready(function () {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        columns: [
            ...
            { data: 'total_invoice', name: 'total_invoice', className: 'text-center' },
            { data: 'total_sell_return', name: 'total_sell_return', className: 'text-center' },
            { data: 'opening_balance_due', name: 'opening_balance_due', className: 'text-center' },
            { data: 'due', name: 'due', className: 'text-center' },
        ],
    ...
    var stock_report_cols = [
        ...
        { data: 'sku', name: 'variations.sub_sku', className: 'text-center' },
        { data: 'product', name: 'p.name' },
        { data: 'variation', name: 'variation', className: 'text-center' },
        { data: 'category_name', name: 'c.name', className: 'text-center' },
        ...
        { data: 'stock', name: 'stock', searchable: false, className: 'text-center' },
    ];
    ...
    register_report_table = $('#register_report_table').DataTable({
        // Total Cheques, the seven configurable payment types (Debit Card,
        // Credit Note, Advance, PayLater, QR, TT Settlement, ...) and Other
        // Payments. Open/Close time, Location and User stay ranged left.
        columnDefs: [
            { targets: [5, 9, 10, 11, 12, 13, 14, 15, 16], className: 'text-center' },
        ],
        processing: true,
        serverSide: true,
        scrollY: '75vh',
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            for (var r in data) {
                ...
                total_bank_transfer_payment += $(data[r].total_bank_transfer_payment).data(
                    'orig-value'
                )
                    ? parseFloat($(data[r].total_bank_transfer_payment).data('orig-value'))
                    : 0;
    ...
    product_purchase_report = $('table#product_purchase_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'sub_sku', name: 'v.sub_sku', className: 'text-center' },
            { data: 'supplier', name: 'c.name' },
            { data: 'ref_no', name: 't.ref_no', className: 'text-center' },
            {
            ...
            { data: 'subtotal', name: 'subtotal', searchable: false, className: 'text-center' },
        ],
        fnDrawCallback: function (oSettings) {
    ...
    if ($('table#product_sell_report_table').length == 1) {
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            columns: [
                ...
                { data: 'contact_id', name: 'c.contact_id', className: 'text-center' },
                { data: 'contact_no', name: 'c.mobile', className: 'text-center' },
                { data: 'contact_email', name: 'c.email' },
                { data: 'invoice_no', name: 't.invoice_no', className: 'text-center' },
                {
                    data: 'transaction_date',
                    name: 't.transaction_date',
                    className: 'text-center',
                ...
                { data: 'tax', name: 'tax_rates.name', className: 'text-center' },
                {
    ...
    //Sell Payment Report
    if ($('table#sell_payment_report_table').length == 1) {
        var spr_cf = window.spr_custom_field_visible || {
            cf1: false,
            cf2: false,
            cf3: false,
            cf4: false,
        ...
        sell_payment_report = $('table#sell_payment_report_table').DataTable({
            // Reference No., Paid On, Amount, Age, Contact ID, Sell.
            columnDefs: [
                { targets: [1, 2, 3, 4, 6, 9], className: 'text-center' },
            ],
            processing: true,
            serverSide: true,
            fixedHeader: false,
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.user_id = $('select#spr_user_id').val();
                    var start = '';
                    var end = '';
                    ...
                    d.start_date = start;
                    d.end_date = end;
                    d.custom_field_1 = $('input#spr_custom_field_1').val();
                    d.custom_field_2 = $('input#spr_custom_field_2').val();
                    d.custom_field_3 = $('input#spr_custom_field_3').val();
                    d.custom_field_4 = $('input#spr_custom_field_4').val();
                    d.age_filter = $('select#spr_age_filter').val();
                },
            },
            columns: [
                ...
                { data: 'amount', name: 'transaction_payments.amount' },
                {
                    ...
                    render: function (data) {
                        ...
                        return data + ' ' + (data == 1 ? LANG.day : LANG.days);
                    },
                },
                {
                    data: 'customer',
                ...
                { data: 'invoice_no', name: 't.invoice_no' },
                { data: 'user_name', name: 'user_name' },
                { data: 'custom_field_1', name: 't.custom_field_1', visible: spr_cf.cf1 },
                { data: 'custom_field_2', name: 't.custom_field_2', visible: spr_cf.cf2 },
                { data: 'custom_field_3', name: 't.custom_field_3', visible: spr_cf.cf3 },
                { data: 'custom_field_4', name: 't.custom_field_4', visible: spr_cf.cf4 },
                { data: 'action', orderable: false, searchable: false },
            ],
            fnDrawCallback: function (oSettings) {
                var total_amount = sum_table_col($('#sell_payment_report_table'), 'paid-amount');
                // With scrollX enabled DataTables clones the tfoot (duplicate #footer_total_amount),
                // so update every copy by class within the wrapper and convert the whole wrapper.
                $('#sell_payment_report_table_wrapper .footer_total_amount').text(total_amount);
                __currency_convert_recursively($('#sell_payment_report_table_wrapper'));
            },
            createdRow: function (row, data, dataIndex) {
                if (!data.transaction_id) {
                    $(row).find('td:eq(0)').addClass('details-control');
        ...
        $('#sell_payment_report_table').on('draw.dt', function () {
        ...
        $('#spr_age_filter').on('change', function () {
            sell_payment_report.ajax.reload();
        });
        ...
        $('#spr_custom_field_1, #spr_custom_field_2, #spr_custom_field_3, #spr_custom_field_4').on(
            'change',
            function () {
                sell_payment_report.ajax.reload();
            }
        ...
        $('#spr_user_id').on('change', function () {
            sell_payment_report.ajax.reload();
        });
    }
    ...
    items_report_table = $('#items_report_table').DataTable({
        columnDefs: [
            { targets: [1, 3, 4, 7, 8, 11, 12, 13, 14], className: 'text-center' },
        ],
        processing: true,
        serverSide: true,
        fixedHeader: false,
        //15 columns otherwise compress into the card and wrap their headers;
        //scroll horizontally instead, as the purchase and sell lists do
        scrollX: true,
        scrollCollapse: true,
        ajax: {
    ...
    $('#tax_report_location_id, #tax_report_date_range, #tax_report_contact_id').change(
        function () {
            updateTaxReport();
        }
    );
...
function updateRegisterReport() {
    var start = $('#register_report_date_range')
        .data('daterangepicker')
        .startDate.format('YYYY-MM-DD');
    var end = $('#register_report_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD');
...
function updateTaxReport() {
    var start = $('#tax_report_date_range').data('daterangepicker').startDate.format('YYYY-MM-DD');
    var end = $('#tax_report_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD');
