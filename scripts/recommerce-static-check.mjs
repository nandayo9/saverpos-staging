...
const purchaseReturnController = read('app/Http/Controllers/PurchaseReturnController.php');
...
const stableLabelMigration = read('Modules/Recommerce/Database/Migrations/2026_08_31_000001_add_encrypted_material_to_recommerce_scan_tokens.php');
...
const adminSidebarMenu = read('app/Http/Middleware/AdminSidebarMenu.php');
const deviceIndexView = read('Modules/Recommerce/Resources/views/device/index.blade.php');
const scanIndexView = read('Modules/Recommerce/Resources/views/scans/index.blade.php');
const inspectionIndexView = read('Modules/Recommerce/Resources/views/inspection/index.blade.php');
const recommerceReadme = read('Modules/Recommerce/README.md');
...
assert(purchaseCreateView.includes('Stock entry') && purchaseCreateView.includes('identify any tracked physical devices'), 'purchase create must explain the stock-to-device receiving handoff');
...
assert(purchaseController.includes('Identification complete') && purchaseController.includes("'Scan '") && purchaseIndexView.includes('Device receiving'), 'the Purchase workspace must expose the purchase-led device-identification handoff');
assert(purchaseController.includes('Awaiting stock receipt') && purchaseController.includes('Receiving issue'), 'the Purchase workspace must distinguish not-yet-actionable and inconsistent receiving states');
assert(!purchaseController.includes("identified · '.$ready.' ready"), 'the Purchase list must not mix inspection readiness into receiving progress');
assert(purchaseController.includes("($remaining === 1 ? '' : 's')") && purchaseIndexView.includes("$notice['remaining'] === 1 ? '' : 's'"), 'the Purchase workspace must use singular device copy for one remaining unit');
assert((purchaseController.match(/assertPurchaseMayBeChanged/g) || []).length >= 3, 'identified-device provenance must block purchase edit, deletion, and status changes');
assert((purchaseReturnController.match(/assertPurchaseMayBeChanged/g) || []).length >= 2, 'identified-device provenance must block both purchase-return display and submission');
assert(purchaseReturnController.includes("'msg' => $e->getMessage()"), 'blocked purchase returns must preserve actionable operator guidance');
assert(recommerceReadme.includes('administrative metadata and stock-affecting purchase lines in one operation'), 'the strict identified-purchase edit policy must document the native coupling that makes granular edits unsafe');
assert(adminSidebarMenu.includes("route('recommerce.inspection.index')") && adminSidebarMenu.includes("'Inspection Queue'"), 'normal Device navigation must expose the permission-gated Inspection Queue');
assert(!adminSidebarMenu.includes("route('recommerce.receiving.index'),\n                                'Purchase receiving'"), 'normal navigation must not expose the generic receiving dead end');
assert(scanIndexView.includes('Find Device') && scanIndexView.includes('Receive stock from Purchases'), 'existing-device scan must be clearly separated from supplier intake');
assert(deviceIndexView.includes('Find and investigate an existing physical device') && !deviceIndexView.includes('Add Device'), 'Device Registry must remain an existing-device lookup, not normal intake');
assert(inspectionIndexView.includes('Awaiting inspection') && inspectionIndexView.includes('Pass inspection'), 'Inspection Queue must use staff-facing status and action labels');
...
assert(labelController.includes('issueOrReuseForLabel'), 'label print path must reuse the durable QR identity before rendering');
...
assert(tokenIssuanceService.includes('raw_token_encrypted'), 'new QR identities must retain only encrypted material for exact reprints');
assert(stableLabelMigration.includes('raw_token_encrypted'), 'stable label migration must add encrypted QR material');
...
assert(blade.includes("lineInspection.style.display = ''"), 'purchase-led receiving must reveal inspection progress after the first accepted device');
