...
`https://test.saver.my` for a fictional team-test estate. It is not a
...
iCore cPanel Git Version Control or cPanel upload
        |
        v
test.saver.my -> Laravel public/ document root -> iCore MySQL database
```
...
1. Create the subdomain `test.saver.my` and set its document root to the
...
4. Enable the free SSL certificate for `test.saver.my` and wait until HTTPS
...
Open `https://test.saver.my/login` and verify:
