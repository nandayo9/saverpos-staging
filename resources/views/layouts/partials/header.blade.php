...
<div
    class="sb-command-header tw-transition-all tw-duration-5000 tw-border-b theme-header-bg tw-shrink-0 lg:tw-h-15 tw-border-primary-500/30 no-print">
    <div class="tw-px-5 tw-py-3">
        <div class="tw-flex tw-items-start tw-justify-between tw-gap-6 lg:tw-items-center">
            <div class="sb-command-header__navigation tw-flex tw-items-center tw-gap-3">
                <button type="button" 
            ...
            <div class="sb-command-header__actions tw-flex tw-flex-wrap tw-items-center tw-justify-end tw-gap-3">
                ...
                {{-- Calculator history logic is in calculator.js (bundled into vendor.js) --}}

                @if (in_array('pos_sale', $enabled_modules))
                    @can('sell.create')
                        <a href="{{ action([\App\Http\Controllers\SellPosController::class, 'create']) }}"
                            class="sb-command-header__primary-action sm:tw-inline-flex tw-transition-all tw-duration-200 tw-gap-2 theme-btn-bg tw-py-1.5 tw-px-3 tw-rounded-lg tw-items-center tw-justify-center tw-text-sm tw-font-medium tw-ring-1 tw-ring-white/10 hover:tw-text-white tw-text-white">
                ...
                <details class="sb-account-menu tw-dw-dropdown tw-relative tw-inline-block tw-text-left">
                    <summary data-toggle="popover"
                        ...
                        @php $__profile_media = Auth::user()->media; @endphp
                        @if (!empty($__profile_media))
                            <img src="{{ $__profile_media->display_url }}"
                                alt="{{ Auth::User()->first_name }} {{ Auth::User()->last_name }}"
                                class="tw-size-5 tw-rounded-full tw-object-cover">
                        @else
                    ...
                    <ul class="sb-account-menu__panel tw-p-2 tw-w-48 tw-absolute {{ $is_rtl ? 'tw-left-0' : 'tw-right-0' }} tw-z-10 tw-mt-2 tw-origin-top-right tw-bg-white tw-rounded-lg tw-shadow-lg tw-ring-1 tw-ring-gray-200 focus:tw-outline-none"
                        ...
                        <div class="sb-account-menu__meta tw-px-4 tw-pt-3 tw-pb-1 tw-flex tw-items-center tw-justify-between tw-gap-2" role="none">
                            ...
                            @if (!empty($__profile_media))
                                <img src="{{ $__profile_media->display_url }}"
                                    alt="{{ Auth::User()->first_name }} {{ Auth::User()->last_name }}"
                                    class="tw-size-9 tw-rounded-full tw-object-cover tw-shrink-0">
                            @else
                                <svg  xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="tw-size-5 tw-shrink-0 tw-text-gray-600"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" /><path d="M12 10m-3 0a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" /><path d="M6.168 18.849a4 4 0 0 1 3.832 -2.849h4a4 4 0 0 1 3.834 2.855" /></svg>
                            @endif
                        </div>
