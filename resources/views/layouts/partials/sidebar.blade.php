...
<aside class="side-bar tw-relative tw-hidden tw-h-full tw-bg-white tw-w-64 xl:tw-w-64 lg:tw-flex lg:tw-flex-col tw-shrink-0">
    ...
    <a href="{{route('home')}}"
        class="tw-flex tw-items-center tw-justify-start tw-gap-3 tw-w-full tw-border-r tw-h-15 theme-logo-bg tw-shrink-0 tw-border-primary-500/30 tw-px-4">
        <img src="{{ asset('img/saverpos-logo.png') }}" alt="SAVERPOS logo" class="saverpos-logo saverpos-logo--sidebar">
        <p class="tw-text-lg tw-font-medium tw-text-white side-bar-heading tw-text-left tw-leading-tight">
            SAVERPOS <span class="tw-inline-block tw-w-2.5 tw-h-2.5 tw-bg-green-400 tw-rounded-full" title="Online"></span>
            <small class="tw-block tw-text-[10px] tw-font-normal tw-text-slate-300 tw-truncate tw-max-w-[150px]">{{ Session::get('business.name') }}</small>
        </p>
    </a>
    ...
    <div class="sb-sidebar-status tw-shrink-0" aria-label="Workspace status">
        <div class="sb-sidebar-status__brand">
            <img src="{{ asset('img/saverpos-logo.png') }}" alt="" aria-hidden="true" class="saverpos-logo sb-sidebar-status__logo">
            <div class="tw-min-w-0">
                <p class="sb-sidebar-status__name">SAVERPOS</p>
                <p class="sb-sidebar-status__business">{{ Session::get('business.name') }}</p>
            </div>
        </div>
        <p class="sb-sidebar-status__meta">
            <span class="sb-sidebar-status__dot" aria-hidden="true"></span>
            Online <span aria-hidden="true">&middot;</span> V{{ config('author.app_version') }}
        </p>
    </div>
