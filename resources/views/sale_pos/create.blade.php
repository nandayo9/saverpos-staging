...
@section('javascript')
    ...
    <script>
        $(document).on('click', '#capture_walk_in', function () {
            var $button = $(this);
            $button.prop('disabled', true);
            $.post('{{ route('walk-ins.store') }}', {location_id: $('#location_id').val()})
                .done(function (response) {
                    $('#walk_in_id').append($('<option>', {value: response.walk_in.id, text: response.walk_in.label, selected: true}));
                    toastr.success(response.message);
                })
                .fail(function (xhr) {
                    toastr.error(xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Unable to record the walk-in.');
                })
