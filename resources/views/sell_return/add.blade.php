								...
								<td>
									...
									@if(config('recommerce.enabled'))
										<div class="form-group" style="margin-top:8px">
											<label class="control-label"><i class="fa fa-barcode"></i> Returned device code(s)</label>
											<textarea class="form-control input-sm" rows="2" name="products[{{$loop->index}}][recommerce_device_codes]" placeholder="Scan the original SaverBro device code"></textarea>
											<input type="hidden" name="products[{{$loop->index}}][recommerce_return_state]" value="RETURNED_PENDING_INSPECTION">
											<input type="hidden" name="products[{{$loop->index}}][product_id]" value="{{$sell_line->product_id}}">
											<input type="hidden" name="products[{{$loop->index}}][variation_id]" value="{{$sell_line->variation_id}}">
										</div>
									@endif
								</td>
