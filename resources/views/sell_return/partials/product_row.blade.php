<tr class="product_row">
	<td>
		...
		@if(config('recommerce.enabled'))
			<div class="form-group" style="margin-top:8px">
				...
				<textarea class="form-control input-sm" rows="2" name="products[{{$row_count}}][recommerce_device_codes]" placeholder="Scan the original SaverBro device code"></textarea>
				<input type="hidden" name="products[{{$row_count}}][recommerce_return_state]" value="RETURNED_PENDING_INSPECTION">
			</div>
		@endif
	...
	<td>
		...
		<input type="text" class="w-auto form-control pos_quantity input_number mousetrap" value="{{@num_format($product->quantity_ordered)}}" name="products[{{$row_count}}][quantity]" 
