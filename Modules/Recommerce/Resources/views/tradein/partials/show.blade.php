@php
    ...
    $catalogueVariation=$valuation->variation;
    $catalogueOrigin=optional($catalogueVariation)->tradeInCatalogueOrigin;
...
@endif
    <div class="sb-ti-panel">
        ...
        <div class="sb-ti-panel-body">
            <div class="row"><div class="col-sm-6"><h4>Device</h4><p>{{ collect([optional($inspection)->cpu,optional($inspection)->ram,optional($inspection)->storage,optional($inspection)->gpu])->filter()->implode(' · ') ?: 'Specification not recorded' }}</p><p class="text-muted">Grade {{ optional($inspection)->cosmetic_grade ?: data_get($valuation->inspection_json,'cosmetic_grade','—') }} · Battery {{ data_get($valuation->inspection_json,'battery_health_percent')!==null?data_get($valuation->inspection_json,'battery_health_percent').'%':'not verified' }}</p></div><div class="col-sm-6"><h4>Catalogue</h4><p>{{ optional(optional($catalogueVariation)->product)->name ?: 'Catalogue record unavailable' }}</p><p class="text-muted">SKU: {{ optional($catalogueVariation)->sub_sku ?: '—' }} · Catalogue origin: {{ $catalogueOrigin?'Trade-In':'Standard' }}</p>@if($catalogueOrigin)<p class="text-muted">Created {{ optional($catalogueOrigin->created_at)->format('d M Y') ?: '—' }} by {{ optional($catalogueOrigin->createdBy)->first_name ?: 'an authorised user' }}{{ $catalogueOrigin->created_from_quick_quote_id?' · Source quote QQ-'.str_pad($catalogueOrigin->created_from_quick_quote_id,5,'0',STR_PAD_LEFT):'' }}</p>@endif<h4>Customer expectation</h4><p style="font-size:20px;font-weight:700">{{ $valuation->customer_requested_amount===null?'Not stated':'RM '.$money($valuation->customer_requested_amount) }}</p><p class="text-muted">Acquisition source: {{ $valuation->acquisition_type==='TRADE_IN'?'Trade-In toward a sale':'Sell-to-SaverBro' }}</p></div></div>
        </div>
    </div>
    <div class="sb-ti-panel">
        ...
        <div class="sb-ti-panel-body">
            ...
            <details style="margin-top:15px"><summary>How the recommendation is calculated</summary><div class="sb-ti-summary" style="max-width:560px"><div class="sb-ti-summary-row"><span>Expected resale</span><strong>RM {{ $money($valuation->expected_resale_amount) }}</strong></div><div class="sb-ti-summary-row"><span>Refurbishment</span><strong>− RM {{ $money($valuation->expected_refurbishment_amount) }}</strong></div><div class="sb-ti-summary-row"><span>Warranty reserve</span><strong>− RM {{ $money(data_get($snapshot,'components.warranty_reserve_amount')) }}</strong></div><div class="sb-ti-summary-row"><span>Logistics handling</span><strong>− RM {{ $money(data_get($snapshot,'components.logistics_handling_amount')) }}</strong></div><div class="sb-ti-summary-row"><span>Inventory risk</span><strong>− RM {{ $money(data_get($snapshot,'components.inventory_risk_amount')) }}</strong></div><div class="sb-ti-summary-row"><span>Required contribution</span><strong>− RM {{ $money(data_get($snapshot,'components.required_contribution_amount')) }}</strong></div></div></details>
        </div>
    </div>
